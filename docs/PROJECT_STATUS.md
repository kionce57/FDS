...
- ✅ A.5: 文件更新 (CLAUDE.md, README.md, docs/)
- ✅ A.6: Keypoint 格式相容性測試 (`test_yolo11_compatibility.py`)

**Phase B（KeypointSmoother 時序過濾）- 未開始:**
...
#### Task 23: Post-Event Recording ✅

- **日期：** 2026-01-05 ~ 2026-01-07
- **狀態：** ✅ 已完成
- **Commits：** `8bfa0e2`, `fb6ba08`, `a787ea3`
- **內容：** 延遲錄製機制 (`threading.Timer`)，事件確認後繼續錄製 n 秒
- **修改檔案：**
  - `src/events/clip_recorder.py` - 實作 FallEventObserver with RollingBuffer
  - `src/capture/rolling_buffer.py` - thread-safe `get_clip()`
  - `config/settings.yaml` - 新增 `clip_after_sec` 設定
- **計畫文檔：** `docs/plans/archive/2026-01-05-post-event-recording.md`
...
1. **Temporal Classifier / ML 模型整合**（1-2 週）

   - TCN 或 LSTM 時序分類器
   - 替換規則引擎為 ML 推論
   - 詳見：`docs/plans/2026-01-05-temporal-classifier-evaluation.md`
...
3. **遠端存取 / 監控 App**（1-2 週）
   ...
   - 雙管線架構（串流 + 偵測）
   - 詳見：`docs/plans/2026-01-06-home-monitoring-app-draft.md`
